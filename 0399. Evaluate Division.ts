...
export function calcEquation (
  ...
  for (const [first, second] of queries) {
    const firstNode = graph.getNode(first)
    const secondNode = graph.getNode(second)
  ...
  function dfs (
    ...
    start.visiting()
    for (const [neighbor, weight] of start.neighbors) {
      if (neighbor.isUnvisited) {
        const retVal = dfs(neighbor, end, product * weight)
        if (retVal !== -1) {
          return retVal
        }
      }
